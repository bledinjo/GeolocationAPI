# GeolocationApi
Geolocation Api JavaScript using ***getCurrentPosition*** method\\

Run the code and move in different locations to get the **latitude** and **longitude** of your current position

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title></title>

</head>
<body>
    <div id="geoResults">
        <h1>Geolocation - Using the getCurrentPosition method</h1>
        <p>Current Location is:</p>
    </div>

    <pre><code>
    <strong>Method receives a (position object)  </strong>
    window.onload = function () {
    var geoLocator = window.navigator.geolocation;
    var posOptions = {enableHighAccuracy: true,timeout: 45000};
    geoLocator.getCurrentPosition(successPosition, errorPosition,
    posOptions);
    }
    function successPosition(pos) {
    alert(pos);
    }
    function errorPosition(err) {
    alert(err);
    }

    <strong>Latitude: and Longitude:  </strong>
      function successPosition(pos) {
        var sp = document.createElement("p");
        sp.innerText = "Latitude: " + pos.coords.latitude +
        " Longitude: " + pos.coords.longitude;
        document.getElementById("geoResults").appendChild(sp);
    }
    function errorPosition(err) {
        var sp = document.createElement("p");
        sp.innerText = "error: " + err.message; + " code: " + err.code;
        document.getElementById("geoResults").appendChild(sp);
    }
        </code></pre>
    <script type="text/javascript">
    window.onload = function () {
        var geoLocator = window.navigator.geolocation;
        var posOptions = { enableHighAccuracy: true, timeout: 45000 };
        geoLocator.getCurrentPosition(successPosition, errorPosition,
        posOptions);
    }
    function successPosition(pos) {
        alert(pos);
    }
    function errorPosition(err) {
        alert(err);
    }
    function successPosition(pos) {
        var sp = document.createElement("p");
        sp.innerText = "Latitude: " + pos.coords.latitude +
        " Longitude: " + pos.coords.longitude;
        document.getElementById("geoResults").appendChild(sp);
    }
    function errorPosition(err) {
        var sp = document.createElement("p");
        sp.innerText = "error: " + err.message; + " code: " + err.code;
        document.getElementById("geoResults").appendChild(sp);
    }
    </script>
</body>
</html>
```
